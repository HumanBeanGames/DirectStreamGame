...
<body>
  <aside>
    ...
    <form id="form">
      ...
      <fieldset>
        <legend>Nearest Palette Bias</legend>
  ...
  <script>
    ...
    function palettePreviewDistanceSquared(a, b, bias) {
      const dl = a.l - b.l;
      const dc = a.c - b.c;
      const dh = Math.sin(hueDeltaRadians(a.h, b.h) * 0.5) * 2 * Math.max(a.c, b.c);
    ...
    function nearestPaletteColor(color, paletteOklch, paletteColors, bias) {
      const oklch = oklabToOklch(rgbToOklab(color[0], color[1], color[2]));
      let bestIndex = 0;
    ...
    function drawRoundedSrgbComparison(srgbRendered, palette, bias) {
      const paletteColors = palette.colors;
      const paletteOklch = paletteColors.map(color => oklabToOklch(rgbToOklab(color[0], color[1], color[2])));
      const roundedPixels = srgbRendered.pixelColors.map(color => nearestPaletteColor(color, paletteOklch, paletteColors, bias));
    ...
    async function bakeMapBlob(artifact) {
      ...
      for (let r = 0; r < 256; r++) {
        for (let g = 0; g < 256; g++) {
          for (let b = 0; b < 256; b++) {
            entries[cursor++] = nearestPaletteIndexForMap(rgbToOklab(r, g, b), paletteOklch, matching);
          }
        }
        if (r % 4 === 0) {
      ...
      writeU64(header, 8, paletteHash(artifact.colors, matching));
    ...
    function nearestPaletteIndexForMap(oklab, paletteOklch, matching) {
      const color = oklabToOklch(oklab);
      let bestIndex = 0;
      let bestDistance = Number.POSITIVE_INFINITY;
    ...
    function paletteHash(colors, matching) {
      ...
      for (const value of [matching.lightness, matching.chroma, matching.hue]) {
    ...
    function previewOnly() {
      ...
      const roundedSrgbRendered = drawRoundedSrgbComparison(srgbRendered, palette, settings.bias);
